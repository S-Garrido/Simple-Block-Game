...
public abstract class GameModel { //Constructs game
	 ...
	 */
	public GameModel(int rows, int cols) {	//Create empty game board
	 ...
	 */
	public void setBoardCell(int rowIndex, int colIndex, BoardCell boardCell) { // Set specific box to a value
		BoardCell[][] newBoard;
		newBoard = new BoardCell[board.length][board[0].length];	//Deep Copy
		for(int row = 0; row < newBoard.length; row ++) {
			for(int col = 0; col < newBoard[row].length; col ++) {
				if(row == rowIndex && col == colIndex) {
					newBoard[rowIndex][colIndex] = boardCell;
				} else {
					newBoard[row][col] = board[row][col];
				}
			}
		}	
		board = newBoard;			//replace old board with deep copy	
	}
	 ...
	 */
	public BoardCell getBoardCell(int rowIndex, int colIndex) { //return specific box
